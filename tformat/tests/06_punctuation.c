...
TEST (punctuation_no_spaces) {
    ...
    char buf[1000];
    strncpy (buf,
	     "He said: ``who is it?'' And, pause, something else!\n"
	     "Here's a list: one; two; three; and four.\n"
	     , 1000);
    t_format(buf, &opts);
    CHECK_STRING_CMP(buf,==,
		     "He said: ``who is it?'' And, pause, something else!\n"
...
TEST (multi_punctuation_with_spaces) {
    struct t_opts opts;
    opts.format_paragraphs = 0;
    opts.format_punctuation = 1;
    opts.max_line_len = 0;
...
MAIN_TEST_DRIVER(one_word,
		 ...
		 punctuation_with_spaces,
		 multi_punctuation_with_spaces);
